                    ...
                    <div class="modal fade" id="exampleModal" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-scrollable modal-md">
                          <div class="modal-content rounded-1">
                            ...
                            <form action="{{ route('localChurch.calling.store') }}" method="post">
                                @csrf
                                <div class="message-body" data-simplebar="">
                                    <div id="message" class="text-center mb-3" role="alert">
                                      Search for a member to assign to this calling....
                                    </div>
                                ...
                                <div class="modal-dialog modal-md">
                                    <div class="modal-content modal-filled bg-light-danger">
                                        <div class="modal-body p-4">
                                            <form action="{{ route('localChurch.calling.destroy',$item->id) }}" method="post">
                                                @csrf
                                                @method('DELETE')
                            ...
                            <div class="modal fade" id="editModel{{ $item->id }}" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-scrollable modal-sm">
                                  <div class="modal-content rounded-1">
                                    ...
                                    <div class="modal-body">
                                    <form action="{{ route('localChurch.calling.update',$item->id) }}" method="post">
                                        @csrf
                                        @method('PUT')
                                        <div class="mb-3">
                                            <label for="message-text" class="control-label mb-2">Category:</label>
                                            <select class="form-select" name="category_id"
                                                style="width: 100%; height: 36px">
                                                @foreach(__('message.callings') as $i)
                                                    <option {{ $item->category_id == $i['id'] ? 'selected' : '' }} value="{{ $i['id'] }}">
                                                    {{ $i['name'] }}
                                                    </option>
                                                @endforeach
                                            </select>
