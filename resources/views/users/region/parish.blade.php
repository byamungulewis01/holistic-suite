                ...
                <div class="d-flex align-items-center">
                    ...
                    <div class="modal fade" id="addModel" tabindex="-1" aria-labelledby="exampleModalLabel1">
                        <div class="modal-dialog" role="document">
                            ...
                            <form action="{{ route('region.parishUser.store') }}" method="POST">
                                <div class="modal-body">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select name="parish" id="parish" class="form-select" required>
                                                ...
                                                <option {{ old('parish') == $item->id ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('parish')
                                            <span class="text-danger">{{ $message }}</span>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="" data-bs-toggle="modal" data-bs-target="#editOffice" class="btn btn-sm btn-outline-primary editOffice">
                                <span data-id="{{ $item->id }}" data-name = "{{ $item->name }}" data-phone = "{{ $item->phone }}" data-email = "{{ $item->email }}" data-username = "{{ $item->username }}" data-parish="{{ $item->parish_id }}">
                                Edit</span>
                            </a>
                            <form action="{{ route('region.parishUser.destroy', $item->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
    ...
    <div class="modal-dialog" role="document">
        ...
        <form id="updateform" method="POST">
            <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="region" class="control-label">Parish:</label>
...
<script>
    $(document).on('click', '.editOffice', function () {
            ...
            var route = "{{ route('region.parishUser.update', ['id' => ':id']) }}";
