                ...
                <div class="d-flex align-items-center">
                    ...
                    <div class="modal fade" id="addModel" tabindex="-1" aria-labelledby="exampleModalLabel1">
                        <div class="modal-dialog" role="document">
                            ...
                            <form action="{{ route('region.localChurchUser.store') }}" method="POST">
                                        ...
                                        <div class="row mb-3">
                                            <div class="col-md-6">
                                                ...
                                                <select class="form-select" name="parish" style="width: 100%; height: 36px" required>
                                                    ...
                                                    <option {{ old('parish') == $item->reg_number ? 'selected' : '' }} value="{{ $item->reg_number }}">{{ $item->name }}</option>
                                                    @endforeach
                                                </select>
                                                @error('parish')
                                                <span class="text-danger">{{ $message }}</span>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="" data-bs-toggle="modal" data-bs-target="#editOffice" class="btn btn-sm btn-outline-primary editOffice">
                                <span data-id="{{ $item->id }}" data-name = "{{ $item->name }}" data-phone = "{{ $item->phone }}" data-email = "{{ $item->email }}" data-username = "{{ $item->username }}"
                                data-parish= "{{ $parish }}" data-localchurch="{{ $localChurch }}" data-localchurchname="{{ $localChurchName }}"
                                >
                                Edit</span>
                            </a>
                            <form action="{{ route('region.localChurchUser.destroy', $item->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
        ...
        <form id="updateform" method="POST">
            <div class="modal-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label for="message-text" class="control-label">Parish:</label>
                            <select class="form-select" name="parish" id="parish" style="width: 100%; height: 36px" required>
                                <option>Select</option>
                                @foreach ($parishes as $item)
...
<script>
    $(document).on('click', '.editOffice', function () {
            ...
            $('#editOffice').find('#parish').val(parish);
            ...
            var route = "{{ route('region.localChurchUser.update', ['id' => ':id']) }}";
