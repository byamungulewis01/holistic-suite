...
<div id="note-full-container" class="note-has-grid row mb-3">
    <div class="col-md-6 single-note-item all-category">
        <div class="card card-body">
            ...
            <h5 class="note-title text-truncate w-75 mb-3" data-noteheading="Icyifuzo cyo gusengera">Gutanga
                Igitekerezo,Ikifuzo cyangwa Ikibazo
            </h5>
    ...
    <div class="col-lg-6">
        <div class="card">
            <form action="{{ route('member.request.storeLeaderMeetRequest') }}" method="POST">
                @csrf
                <div class="card-header">
                    <h5 class="card-title mb-0">gitekerezo,Ikifuzo cyangwa Ikibazo</h5>
                </div>
                <div class="card-body">
                    <div class="row mb-3">
                        <div class="col-md-6">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-lg-6">
                            <label for="leader" class="form-label">Umuyobozi <span class="text-danger">*</span></label>
                            <select name="leader" class="form-select" id="leader" style="height: 36px; width: 100%"
                                required>
                                <option value="" selected disabled>Select</option>
                                @foreach (__('client/words.leaders') as $item)
                                <option {{ old('leader')==$item['id'] ? 'selected' : '' }} value="{{ $item['id'] }}">{{
                                    $item['name'] }}</option>
                                @endforeach
                            </select>
                            @error('service')
                    ...
                    <div class="col-lg-12">
                        <div>
                            <label for="exampleFormControlTextarea1" class="form-label">Impamvu <span
                                    class="text-danger">*</span></label>
                            <textarea name="reason" class="form-control" placeholder="Impamvu Ushakira umuyobozi" required
