...
class AuthenticationController extends Controller
{
    ...
    public function loginAuth(Request $request)
    {
        ...
        if (auth()->guard('member')->attempt($credentials)) {
            ...
            } elseif (in_array(auth()->guard('member')->user()->member->status, ['2', '3', '4'])) {
                auth()->guard('member')->logout();
                return redirect()->back()->with('error', 'You don\'t have access to ADEPR MIS');
            }
    ...
    public function registerAuth(Request $request)
    {
        ...
        if (in_array($member->status, ['2', '3', '4'])) {
            return redirect()->back()->with('error', 'Unless Active Member will be registered');
        }
